...
\begin{figure}[H]
  ...
  \begin{prooftree}
    \hypo{\Xi;\Gamma;\chi;\sigma;\epsilon\vdash^T_{code}$ l : $\forall$().ctx$}
    \hypo{$ctx $\sim$ \{ $\chi$ | $\sigma\to\epsilon$ \}$}
    \infer2[jump to label]{\Xi;\Gamma;\chi;\sigma;\epsilon\vdash^I$ jmp l $\dashv\chi;\sigma;\epsilon}
  \end{prooftree}
...
\begin{figure}[H]
  ...
  \begin{prooftree}
    \hypo{$r is a register$}
    \hypo{\Xi;\Gamma;\chi;\sigma;\epsilon\vdash^T_{code}$ l : $\forall$().\{ $\hat{\chi}$ | $\sigma\to$ r \}$}
    ...
    \infer1[function call]{\Xi;\Gamma;\chi;\sigma;$ r $\vdash^I$ call l $\dashv\chi;\sigma;$ r$}
  \end{prooftree}
  \\\vspace{\baselineskip}
  \begin{prooftree}
    \hypo{$n $\in\mathbb{N}}
    \hypo{$n $\leq$ p$}
    \hypo{\Xi;\Gamma;\chi;\sigma;\epsilon\vdash^T_{code}$ l : $\forall$().\{ $\hat{\chi}$ | $\sigma\to$ n \}$}
    ...
    \infer1[function call]{\Xi;\Gamma;\chi;\sigma;$ n $\vdash^I$ call l $\dashv\chi;\sigma;$ n$}
  \end{prooftree}
