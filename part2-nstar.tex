...
\begin{figure}[H]
  ...
  \begin{prooftree}
    \hypo{$r is a register$}
    \hypo{$\%sp : sptr ts $\in\Delta}
    \hypo{$n $\leq$ 8 $\in\mathbb{N}}
    \infer[no rule, rule margin=1ex]3{\Gamma\vdash^K$ t : Tn, s : Ts\hspace{1.5em}$%
                                      $t $\nsim$ *\{\}\hspace{1.5em}$%
                                      $ts $\sim$ t::s$}
    \infer1[\texttt{pop} to register]{\Delta,\Xi\vdash^I$ pop r $\dashv\Delta\setminus\{$ r, \%sp $\}$, r : t, \%sp : sptr s$}
  \end{prooftree}
  \\\vspace{\baselineskip}
  \begin{prooftree}
    \hypo{$n $\in\mathbb{N}}
    \hypo{\Gamma\vdash^K$ t : Tn, s : Ts$}
    \infer[no rule, rule margin=1ex]2{$\%sp : sptr ts $\in\Delta$\hspace{1.5em}$%
                                      \Delta\vdash^T_{code}$ o(e) : t\hspace{1.5em}$%
                                      $ts $\sim$ t::s$}
    \infer1[\texttt{pop} to pointer offset]{\Delta,\Xi\vdash^I$ pop o(e) $\dashv\Delta\setminus$ \%sp, \%sp : sptr s$}
  \end{prooftree}
...
\begin{figure}[H]
  ...
  \begin{prooftree}
    \hypo{$c is a context$}
    \hypo{\Gamma\vdash^K$ s : Ts$}
    \hypo{$\%sp : sptr *c::s $\in\Delta}
    \hypo{$c $\subseteq\Delta}
    \infer4{\Delta,\Xi\vdash^I$ ret $\dashv\Delta}
  \end{prooftree}
...
\begin{figure}[H]
  ...
  \begin{prooftree}
    ...
    \hypo{\Gamma\vdash^K$ t : Tn, s : Ts$}
    \hypo{\Delta\vdash^T_{code}$ e : t$}
    \hypo{$\%sp : sptr s $\in\Delta}
    \infer4{\Delta,\Xi\vdash^I$ push e $\dashv\Delta\setminus$ \%sp, \%sp : sptr t::s$}
  \end{prooftree}
