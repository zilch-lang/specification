...
\begin{figure}[H]
  ...
  \begin{prooftree}
    ...
    \infer[no rule, rule margin=1ex]2{\Gamma\vdash^K$ t : Tn, s : Ts\hspace{1.5em}$%
                                      $t $\nsim$ *\{\}\hspace{1.5em}$%
                                      $ts $\sim$ t::s$}
    \infer1[\texttt{pop} to register]{\Delta,$ \%sp : sptr ts$,\Xi\vdash^I$ pop r $\dashv\Delta\setminus$ r, r : t, \%sp : sptr s$}
  \end{prooftree}
  \\\vspace{\baselineskip}
  \begin{prooftree}
    \hypo{$n $\in\mathbb{N}}
    ...
    \hypo{$ts $\sim$ t::s$}
    \infer4[\texttt{pop} to pointer offset]{\Delta,$ \%sp : sptr ts$,\Xi\vdash^I$ pop o(e) $\dashv\Delta$, \%sp : sptr s$}
  \end{prooftree}
...
\begin{figure}[H]
  ...
  \begin{prooftree}
    ...
    \hypo{\Gamma\vdash^K$ t : Tn, s : Ts$}
    \hypo{\Delta\vdash^T_{code}$ e : t$}
    \infer3{\Delta,$ \%sp : sptr s$,\Xi\vdash^I$ push e $\dashv\Delta$, \%sp : sptr t::s$}
  \end{prooftree}
