...
\begin{figure}[H]
  ...
  \framebox[\textwidth][c]{\parbox{\textwidth}{
      ...
      \begin{center}
        ...
        \\\vspace{\baselineskip}
        \begin{prooftree}
          \hypo{\Gamma\vdash^T_{data}$ c$_1$ : t$_1$, c$_2$ : t$_2$, \ldots, c$_p$ : t$_p}
          \infer1{\Gamma\vdash^T_{data}$ (c$_1$, c$_2$ \ldots, c$_p$) : *(t$_1$, t$_2$, \ldots, t$_p$)$}
        \end{prooftree}\hspace{1.5em}
        \begin{prooftree}
          \hypo{$c is a character constant$}
          \infer1{\Gamma\vdash^T_{data}$ c : *s8$}
        \end{prooftree}
        \\\vspace{\baselineskip}
        \begin{prooftree}
          \hypo{$n : $\mathbb{N}}
...
The \texttt{jmp} instruction takes a single parameter: a function label, whose type must subtype the current context $\chi$.
The next context is the same as the current context because our type system does not materialize context switches.
However, this is not an issue because \texttt{jmp} is a terminal instruction.
...
\begin{figure}[H]
  ...
  \framebox[\textwidth][c]{\parbox{\textwidth}{
      ...
      \begin{prooftree}
        ...
        \infer[no rule, rule margin=1ex]3{\sigma=$ t$_0\dblcolon$ t$_1\dblcolon$ \ldots $\dblcolon$ t$_p\dblcolon$ s\hspace{1.5em}$%
                                          \hat{\sigma}=\hat{\text{t}_0}\dblcolon\hat{\text{t}_1}\dblcolon$ \ldots $\dblcolon\hat{\text{t}_p}\dblcolon$ $\hat{\text{s}}$\hspace{1.5em}$%
...
The \texttt{call} instruction takes a single function label as argument.
...
The \texttt{ret} instruction takes no argument, but requires its continuation to be bound in the current context, and that the context of the continuation subtypes the current context (to avoid improperly typed/unbound registers).
Note that it does not allow returning to a stack-continuation because this would lead to writing infinite types (the stack continuation not being dropped, it must be present in the stack of the continuation, leading to such case: $\sigma_{1}\dblcolon\forall().\{ \zeta | \sigma_{1}\dblcolon\ldots\dblcolon\sigma_{2} \to \epsilon \}\dblcolon\sigma_{2}$ where $\ldots$ is the $\forall$-type representing the continuation).
...
\begin{figure}[H]
  ...
  \framebox[\textwidth][c]{\parbox{\textwidth}{
      ...
      \begin{prooftree}
        ...
        \hypo{$n $\in\mathbb{N}}
        \hypo{$n $\leq$ p$}
        \infer[no rule, rule margin=1.0ex]3{\hat{\sigma}=$ t$_0\dblcolon$ t$_1\dblcolon$ \ldots $\dblcolon$ t$_p\dblcolon$ s\hspace{1.5em}$%
          $t$_n\sim\forall$().ctx$}
      ...
      \begin{prooftree}
        ...
        \infer[no rule, rule margin=1.0ex]2{$n $\leq$ p\hspace{1.5em}$%
          $k $\leq8$\hspace{1.5em}$%
          \hat{\sigma}=$ t$_0\dblcolon$ t$_1\dblcolon$ \ldots $\dblcolon$ t$_p\dblcolon$ s\hspace{1.5em}$%
...
The \texttt{sld} instruction takes two arguments: an integer and a register, and requires the integer to be a valid stack index.
...
\begin{figure}[H]
  ...
  \framebox[\textwidth][c]{\parbox{\textwidth}{
      ...
      \begin{prooftree}
        \hypo{$r is a register$}
        \hypo{$n $\in\mathbb{N}}
        \hypo{\hat{\sigma}=$ t$_0\dblcolon$ t$_1\dblcolon$ \ldots $\dblcolon$ t$_p\dblcolon$ s$}
...
The \texttt{sst} instruction takes two arguments: a register and an integer, requiring the integer to be a valid stack index pointing to a 4 or 8 bytes wide cell.
...
\begin{figure}[H]
  ...
  \framebox[\textwidth][c]{\parbox{\textwidth}{
      ...
      \begin{prooftree}
        ...
        \hypo{\sigma^\prime=$ t $\dblcolon\sigma}
...
The \texttt{salloc} instruction allows reserving (allocating) $n$ bytes of memory on top of the stack, for a specific type which is $n$ bytes wide.
Note that according to the \texttt{sst} instruction, the type may be changed later on.
...
\begin{figure}[H]
  ...
  \framebox[\textwidth][c]{\parbox{\textwidth}{
      ...
      \begin{prooftree}
        \hypo{$n $\geq 1}
        \hypo{$n $\leq$ p$}
        \infer[no rule, rule margin=1.0ex]2{\sigma=$ t$_0\dblcolon$ t$_1\dblcolon$ \ldots $\dblcolon$ t$_p\dblcolon$ s\hspace{1.5em}$%
                                            \sigma^\prime=$ t$_1\dblcolon$ \ldots $\dblcolon$ t$_p\dblcolon$ s$}
        \infer1[\texttt{sfree} with stack continuation]{\Xi;\Gamma;\chi;\sigma;$ n $\vdash^I$ sfree $\dashv\chi;\sigma^\prime;$ n$-1}
      \end{prooftree}
      \\\vspace{\baselineskip}
      \begin{prooftree}
        \hypo{\sigma=$ t$_0\dblcolon$ t$_1\dblcolon$ \ldots $\dblcolon$ t$_p\dblcolon$ s$}
        \hypo{\sigma^\prime=$ t$_1\dblcolon$ \ldots $\dblcolon$ t$_p\dblcolon$ s$}
...
The \texttt{sfree} instruction frees the top-most stack cell available unless it contains the current continuation.
...
The \texttt{sref} instruction takes an integer and a register as arguments, and requires the integer to be a valid stack index to a type at most 4 or 8 bytes wide.
...
Note that trying to write to a pointer to stack data which has been freed with \texttt{sfree} (as in Listing~\ref{lst:nstar-instructionset-stack-sref-ubexample}) is Undefined Behavior.
...
The \texttt{ld} instruction takes a register and a pointer offset as arguments, and requires that the value held in the register matches the value behind the pointer.
...
The \texttt{st} instruction takes a value (literal value or register) and a pointer offset as arguments.
...
The register must also not hold a value of unknown type \texttt{!}, because its type is not known.
...
\begin{figure}[H]
  ...
  \framebox[\textwidth][c]{\parbox{\textwidth}{
      ...
      \begin{prooftree}
        ...
        \hypo{\Xi;\Gamma;\chi;\sigma;\epsilon\vdash^T_{code}$ r : $\forall$().$\zeta}
        \infer2[move cont from register to register]{\Xi;\Gamma;\chi;\sigma;$ r $\vdash^I$ mv r, d $\dashv\chi\setminus$\{ r, d \}$,$ d : $\forall$().$\zeta;\sigma;$ d$}
      \end{prooftree}
      \\\vspace{\baselineskip}
      \begin{prooftree}
        \hypo{$r is a register$}
...
The \texttt{mv} instruction takes a value (literal value or register) and a register as arguments, and overwrites the second register with a copy of the first value (if in a register).
...
The \texttt{nop} instruction does absolutely nothing else than skipping a CPU cycle and taking space in the end executable.
