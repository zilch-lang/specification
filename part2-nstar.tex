...
\begin{figure}[H]
  ...
  \begin{prooftree}
    \hypo{\Xi;\chi;\sigma;\epsilon\vdash^T_{code}$ l : $\forall$().ctx$}
    \hypo{$ctx $\sim$ \{ $\chi$ | $\sigma\to\epsilon$ \}$}
    \infer2[jump to label]{\Xi;\chi;\sigma;\epsilon\vdash^I$ jmp l $\dashv\chi;\sigma;\epsilon}
  \end{prooftree}
...
\begin{figure}[H]
  ...
  \begin{prooftree}
    \hypo{$r is a register$}
    \hypo{\Xi;\chi;\sigma;\epsilon\vdash^T_{code}$ l : $\forall$().\{ $\hat{\chi}$ | $\sigma\to$ r \}$}
    \infer[no rule, rule margin=1.0ex]2{\Xi;\hat{\chi};\hat{\sigma};$ r $\vdash^T_{code}$ r : $\forall$().\{ $\chi^\prime$ | $\sigma^\prime\to\epsilon^\prime$ \}\hspace{1.5em}$%
                                        \chi\sim\hat{\chi}}

    \infer1[function call]{\Xi;\chi;\sigma;$ r $\vdash^I$ call l $\dashv\chi;\sigma;$ r$}
  \end{prooftree}
  \\\vspace{\baselineskip}
  \begin{prooftree}
    \hypo{$n $\in\mathbb{N}}
    \hypo{$n $\leq$ p$}
    \hypo{\Xi;\chi;\sigma;\epsilon\vdash^T_{code}$ l : $\forall$().\{ $\hat{\chi}$ | $\sigma\to$ n \}$}
    ...
    \infer1[function call]{\Xi;\chi;\sigma;$ n $\vdash^I$ call l $\dashv\chi;\sigma;$ n$}
  \end{prooftree}
...
\begin{figure}[H]
  ...
  \begin{prooftree}
    \hypo{$r is a register$}
    \hypo{\Xi;\chi;\sigma;$ r $\vdash^T_{code}$ r : $\forall$().\{ $\chi^\prime$ | $\sigma\to\epsilon$ \}$}
    \hypo{\chi\sim\chi^\prime}
    \infer3[return to caller in register]{\Xi;\chi;\sigma;$ r $\vdash^I$ ret $\dashv\chi;\sigma;$ r$}
  \end{prooftree}
  \\\vspace{\baselineskip}
  \begin{prooftree}
    \hypo{\hat{\sigma}=$ t$_0$::t$_1$::\ldots::t$_p$::s$}
    \hypo{$n $\in\mathbb{N}}
    \hypo{$n $\leq$ p$}
    \infer[no rule, rule margin=1.0ex]3{\Xi;\chi;\hat{\sigma};$ n $\vdash^T_{code}$ t$_n$ : $\forall$().\{ $\chi^\prime$ | $\hat{\sigma}\to\epsilon$ \}\hspace{1.5em}$%
                                        \chi\sim\chi^\prime}
    \infer1[return to caller on stack]{\Xi;\chi;\hat{\sigma};$ n $\vdash^I$ ret $\dashv\chi;\hat{\sigma};$ n$}
  \end{prooftree}
...
\begin{figure}[H]
  ...
  \begin{prooftree}
    \infer0{\Xi;\chi;\sigma;\epsilon\vdash^I$ nop $\dashv\chi;\sigma;\epsilon}
  \end{prooftree}
