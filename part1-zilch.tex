...
\begin{figure}[H]
  ...
  \framebox[\textwidth][c]{
    \parbox{\textwidth}{
      ...
      \scalebox{.5}{
        \includegraphics{zilch/types/mixfix}
      }\\
      \scalebox{.5}{
...
\noindent\begin{minted}{\zilchlexer}
  ...
  def const: (u64, u64) -> u64 := fn(x, y) x
\end{minted}
\vspace*{\baselineskip}
...
Parsing infix operators is ambiguous by nature if no decision is made.
Usual programming languages only have a fixed set of infix operators with predefined fixities (left or right associative, with a given precedence) to deal with such ambiguities.
Some programming languages allow defining infix operators, with user-supplied fixities explicitly given in the program.

While Zilch also follows this approach, it only does so for a limited set of operators: infix operators.
Prefix and postfix operators are not ambiguous to parse unless when mixed.
For example, given the two prefix operators \texttt{-\_} and \texttt{$\neg$\_}, \texttt{- $\neg$ 3} only has one possible interpretation: \texttt{- ($\neg$ 3)}.
However, given the prefix operator \texttt{-\_} and the postfix operator \texttt{\_!}, \texttt{- 0 !} has two interpretations: \texttt{- (0 !)} and \texttt{(- 0) !}.
While the same approach could be taken also for prefix and postfix operators, this is not that useful in practice, and is therefore not officially supported.
